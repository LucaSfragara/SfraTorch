...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.source_embedding = SpeechEmbedding(input_dim, d_model, time_reduction, reduction_method= reduction_method, dropout = dropout)
    ...
    def decode(
        ...
        pad_mask_tgt = None
        if target_lengths is not None:
            pad_mask_tgt = PadMask(padded_targets, target_lengths).to(encoder_output.device)
        ...
        causal_mask = CausalMask(padded_targets).to(encoder_output.device)
    ...
    def forward(
        ...
        encoder_output, pad_mask_src, enc_running_att, ctc_inputs = self.encode(padded_sources, source_lengths) #type: ignore
        ...
        return seq_out, running_att, ctc_inputs #type: ignore
    ...
    def score(self, batch_prompts: torch.Tensor, encoder_output: torch.Tensor, pad_mask_src: torch.Tensor) -> torch.Tensor:
        ...
        eff_batch_size = batch_prompts.size(0)
        orig_batch_size = encoder_output.size(0)

        if eff_batch_size != orig_batch_size:
            if eff_batch_size % orig_batch_size != 0:
                 raise ValueError(f"Effective batch size ({eff_batch_size}) must be a multiple of original batch size ({orig_batch_size}) for beam search.")
            beam_width = eff_batch_size // orig_batch_size

            expanded_encoder_output = encoder_output.repeat_interleave(beam_width, dim=0)
            expanded_pad_mask_src = pad_mask_src.repeat_interleave(beam_width, dim=0)
        else:
            expanded_encoder_output = encoder_output
            expanded_pad_mask_src = pad_mask_src

        seq_out, _ = self.decode(batch_prompts, expanded_encoder_output, None, expanded_pad_mask_src)
