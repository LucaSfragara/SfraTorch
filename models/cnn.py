...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for layer in range(self.nlayers):
            ...
            num_input_channels = num_channels[layer]
        ...
        for i in range(self.nlayers):
            cur_width = (cur_width - kernel_sizes[i] + strides[i]) // strides[i]
        self.linear_layer = Linear(num_channels[-1] * cur_width, num_linear_neurons)
    ...
    def forward(self, A):
        ...
        for layer in range(self.nlayers):
            A = self.convolutional_layers[layer].forward(A)
            A = self.activations[layer].forward(A)
    ...
    def backward(self, labels):
        ...
        for layer in range(self.nlayers-1, -1, -1):
            print(layer, grad.shape)
            grad = self.activations[layer].backward(grad)
            grad = self.convolutional_layers[layer].backward(grad)
